                    ...
                    <table id="tablemon_upi" class="datatable-main table table-bordered table-striped">
                        ...
                        <thead class="hdr-yellow">
                            <tr>
                                ...
                                <th colspan="3" class="hdr-wo">KOREKSI BANGSANG</th>
    ...
    $(document).ready(function () {
        ...
        var table = $('#tablemon_upi').DataTable({
            ...
            columns: [
                ...
                { data: 'JTR_KOREKSI', defaultContent: '', render: d => formatNumber(d, 0) },
                { data: 'JTM_KOREKSI', defaultContent: '', render: d => formatNumber(d, 0) },
                { data: 'GRD_KOREKSI', defaultContent: '', render: d => formatNumber(d, 0) },
           ...
           columnDefs: [
                ...
                { targets: 0, className: 'dt-body-right', orderable: false, searchable: false, width: '5px' },  
            ...
            createdRow: function (row, data, dataIndex) {
                $('td', row).each(function (colIndex) {
                    ...
                    const allowedColumns = ['JTR_RENCANA', 'JTM_RENCANA', 'GRD_RENCANA',
                                            ...
                                            'JTR_KOREKSI', 'JTM_KOREKSI', 'GRD_KOREKSI',            
            ...
            try {
                ...
                await XlsxPopulate.fromDataAsync(wbout)
                    .then(function (workbook) {
                        ...
                        const totalCols = headers.length; // 22 kolom (A–V)
                        const lastCol = "V";
                        const lastRow = rows.length;

                        // ===============================
                        // MERGE HEADER (2 LEVEL)
                        // ===============================
                        sheet.range("A5:A6").merged(true);
                        sheet.range("B5:B6").merged(true);
                        sheet.range("C5:C6").merged(true);
                        sheet.range("D5:D6").merged(true);

                        sheet.range("E5:G5").merged(true);   // RENCANA
                        sheet.range("H5:J5").merged(true);   // WORK ORDER
                        sheet.range("K5:M5").merged(true);   // PROSES
                        sheet.range("N5:P5").merged(true);   // KOREKSI
                        sheet.range("Q5:S5").merged(true);   // TERPASANG
                        sheet.range("T5:V5").merged(true);   // PORSENTASE

                        // ===============================
                        // STYLE HEADER (BIRU)
                        // ===============================
                        sheet.range("A5:V6").style({
                            bold: true,
                            fill: "5B8CC9",
                            fontColor: "FFFFFF",
                            border: true,
                        ...
                        // ===============================
                        // FILTER HEADER (sub-header)
                        // ===============================
                        sheet.range("A6:V6").autoFilter();
                        ...
                        // ===============================
                        // BOLD KOLOM D – S (ANGKA)
                        // ===============================
                        const startDataRow = 7; // data dimulai setelah 2 baris header
                        for (let col = 4; col <= 22; col++) {
                            sheet.range(
                                sheet.cell(startDataRow, col),
                                sheet.cell(lastRow, col)
                            ).style({ bold: true });
                        }
...
// X=========================================================================================================================
        ...
        $('#dataModal').on('shown.bs.modal', function () {
            ...
            detailTable = $('#table_mondaf_provupi').DataTable({
                ...
                //     // bersihkan dulu
                //     $tbody.find(
                //         'tr.subtotal-row, tr.start-kontrak-row, tr.end-kontrak-row'
                //     ).remove();
                ...
                //         * START KONTRAK
                //         * ========================== */
                //         if (!prevRow || prevRow.NOMOR_KONTRAK_RINCI !== kontrak) {
                ...
                //             insertSubtotal(rowsNodes[i], lastGroup, subtotal);
                //             subtotal = 0;
                //         }
                ...
                //         * END KONTRAK
                //         * ========================== */
                //         if (!nextRow || nextRow.NOMOR_KONTRAK_RINCI !== kontrak) {
                //             insertSubtotal(
                //                 nextRow ? rowsNodes[i + 1] : null,
                //                 group,
                //                 subtotal
                //             );
                //             insertEndOfKontrak(
                ...
                //     function insertStartOfKontrak(refNode, kontrak) {
                //         if (!kontrak) return;
                ...
                //                     'START OF KONTRAK RINCI : ' + kontrak +
                //                 '</td>' +
                //             '</tr>';
                ...
                //     function insertSubtotal(refNode, group, value) {
                //         if (!group) return;

                //         const totalCols = api.columns().count();
                //         let html = '<tr class="subtotal-row fw-bold">';

                //         for (let i = 0; i < totalCols; i++) {
                //             if (i === 0) {
                //                 html += '<td>TOTAL ' + group + '</td>';
                //             } else if (i === colQtyIndex) {
                //                 html += '<td style="text-align:right; color:darkred;">' + value + '</td>';
                //             } else {
                //                 html += '<td></td>';
                //             }
                //         }
                ...
                //         refNode ? $(refNode).before(html) : $tbody.append(html);
                //     }

                //     function insertEndOfKontrak(refNode, kontrak) {
                //         if (!kontrak) return;
